...
public class Banco {
	...
	int id = contas.size();
	try{
		System.out.println(Thread.currentThread().getName()+": criar conta "+id+" com saldo "+saldo);
	...
	public double fecharConta(int id) throws ContaInvalida{
		...
		this.lockBanco.lock();
		System.out.println(Thread.currentThread().getName()+": fechar conta "+id);
	...
	public double consultar(int id) throws ContaInvalida{
		...
		double saldo= 0;
                ...
                //caso exista, bloquear a conta usando lock
                this.contas.get(id).lock();
                //libertar o lock do banco
                this.lockBanco.unlock();
                
                //retornar o saldo
                saldo=this.contas.get(id).consultar();
                //Libertar o lock da conta
	...
	public double consultarTotal(int[] contasInput) {
		...
		for(int i = 0; i < contasInput.length; i++){
			...
			if(!contas.containsKey(id)){
				//conta não existe
				System.out.println(Thread.currentThread().getName()+": Conta "+id+" não existe! Calcular saldo para as restantes contas.");
			}
			else{
		...
		System.out.println(Thread.currentThread().getName()+": total "+contasLocked+" = "+saldoTotal);
	...
	public void transferir(int conta_origem, int conta_destino, double valor) throws ContaInvalida, SaldoInsuficiente{
		...
		System.out.println(Thread.currentThread().getName()+": transferir "+valor+"€ de "+conta_origem+" para "+conta_destino);
...
class SaldoInsuficiente extends Exception{
	...
	public SaldoInsuficiente(int id){
		super(Thread.currentThread().getName()+": Conta "+id+" não tem saldo suficiente para operação!");
	}
}
...
class ContaInvalida extends Exception{
	...
	public ContaInvalida(int id){
		super(Thread.currentThread().getName()+": Conta "+id+" não existe!");
	}
}
