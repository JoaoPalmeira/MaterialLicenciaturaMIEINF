...
public class BoundedBuffer {
	...
	public synchronized void put(int v){
		try {
			...
			//System.out.println("PUT: notificar threads de que um valor foi colocado no buffer");
			this.notifyAll();
	...
	public synchronized int get(){
		...
		try {
			while(this.poswrite==0){
				...
				//System.out.println("GET: buffer está vazio, esperar até que tenha valor");
				this.wait();
			}
		} catch (InterruptedException e) {
			e.printStackTrace();
		...
		//System.out.println("GET: notificar threads de que um valor foi retirado do buffer");
		this.notifyAll();
