...
public class Main2 {
	...
	public static void main(String[] args) {
		...
		for(P = 1; P <= 9; P++){
			...
			for (int i=0;i<P;i++){
				if(i<P-1){
					tsps[i]=new Thread(new Producer2(b,tp, opsprod));
				}
				else //criar última thread com o resto das operações que faltam
					tsps[i]=new Thread(new Producer2(b,tp, opsprod+rest));	
			...
			for (int i=0;i<C;i++){
				if(i<C-1){
					tscs[i]=new Thread(new Consumer2(b,tc,opscons));
				}
				else //criar última thread com o resto das operações que faltam
					tscs[i]=new Thread(new Consumer2(b,tc,opscons+rest));
