...
void consts(char* value, FILE *fildes){
  ...
  file = open("aux.txt", O_RDONLY, 0644);
  output = open("output.txt", O_CREAT|O_WRONLY, 0644);

  while((nBytes=readln(file,buffer,bufS))>0) {
      buffer[strlen(buffer)-1]='\0';
      strcat(buffer,":");
      strcat(buffer,value);
      strcat(buffer,"\n");
      write(output, buffer, nBytes+9);
  }
  close(file);
  close(output);
}
