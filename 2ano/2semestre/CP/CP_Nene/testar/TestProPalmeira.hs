...
-- union de g10 e g12
g14 :: Graph Char
g14 = Graph {nodes = fromList ['W', 'X', 'Y', 'Z', 'A', 'B', 'C', 'D'], 
            edges = fromList [Edge 'X' 'Y', Edge 'Z' 'X', Edge 'Z' 'Y', Edge 'Z' 'W', Edge 'A' 'B', Edge 'B' 'C', Edge 'D' 'C']
            }
...
test_adj :: Test
test_adj = TestList[adj g1 1 ~?= fromList [Edge 1 1], adj g2 'A' ~?= fromList [Edge 'A' 'B', Edge 'A' 'D'], adj g3 'Y' ~?= fromList [Edge 'Y' 'X', Edge 'Y' 'Z'], 
                    adj g4 'W' ~?= fromList [Edge 'W' 'Y']]  
...
test_isForest :: Test
test_isForest = TestList[Graph.isForest g4~?=False, Graph.isForest g3~?=False, Graph.isForest g10~?=False, Graph.isForest g7~?=False, 
                         Graph.isForest g9~?=True, Graph.isForest g1~?=True]
...
test_path :: Test
test_path = TestList[Graph.path g9 'X' 'A' ~?= Nothing, Graph.path g10 'Z' 'Y' ~?= Just [Edge 'Z' 'Y'], Graph.path g13 1 6 ~?= Just [Edge 1 3, Edge 3 6], 
                     Graph.path g12 'A' 'D' ~?= Nothing]

test_topo :: Test
test_topo = TestList[Graph.topo g3 ~?= [fromList ['W','Y'], fromList ['X'], fromList ['Z']], Graph.topo g9 ~?= [fromList ['A','C','E'], fromList ['D'], fromList ['B']], 
                     Graph.topo g2 ~?= [fromList ['A', 'C'], fromList ['D'], fromList ['B']]]


main = runTestTT $ TestList [test_adj, test_swap, test_isEmpty, test_isValid, test_isDAG, test_isForest, test_isSubgraphOf, test_transpose, test_union, 
