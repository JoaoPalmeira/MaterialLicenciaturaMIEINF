...
--instance (Ord v, Arbitrary v) => Arbitrary (Graph v) where
  --  arbitrary = aux `suchThat` isValid
    --    where aux = do ns <- arbitrary
      --                 es <- arbitrary -- `suchThat` verificArestas
                       --let es1 = verificArestas ns es 

        --               return $ Graph {nodes = fromList ns, edges = fromList es1}
...
--pegar nos nodos das arestas do resultado da bft e juntar aos nodos dados como argumento e tem de dar os nodos do resultado da bft 
--prop_reachable :: Graph Int -> Set Int -> Bool 
--prop_reachable g v = retiraNodos g v 

--prop_isPathOf
--prop_isPathOf

--prop_path :: Graph Int -> Graph Int -> Graph Int -> Bool
--prop_path g1 g2 g3 = if pertence g2 g3 a g1 ==>  
